...
import { getBufferSize, distance, numberToRGB } from '../utils/Utility';
...
export default class EdgeContainer extends PIXI.Container {
    constructor(styleRules) {
        ...
        this.pluginName = 'edge-container';
    ...
    addChild(child) {
        ...
        //edgeAttributes of addRule methods
        const edgeAttributes = (rule) => {
            const attr = rule.edgeAttributes;
            if (attr) {
                if (attr.lineWidth) {
                    child.updateLineWidth(true);
                }
                if (attr.color) {
                    child.updateLineColor(true);
                }
            }
        };
        this.styleRules.forEach((rule) => {
            if (rule.edgeSelector && typeof rule.edgeSelector === 'function') {
                let call = rule.edgeSelector.apply(this, arguments);
                if (!call) {
                    return false;
                }
            }
        ...
        child.setAttributes();
        // this.thicknessArray.set([child.lineWidth], index);
        this.updateLineWidth(child);
