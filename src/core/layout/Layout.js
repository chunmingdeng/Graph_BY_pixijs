/**
 * The base class of Layout.
 */
export default class Layout {
    constructor(nodeSprites, edgeSprites, nodeContainer) {
        ...
        for (let i = 0; i < nodeContainer.instanceCount; i++) {
            ...
            const incomingArray = nodeSprite.incoming.map(incoming => nodeContainer.idIndexMap.indexFrom(incoming.fromId));
            this.incomingArrays.push(...incomingArray);
            incomingIndex += incomingArray.length;

            this.outgoingSlotArray.set([outgoingIndex], i);
            const outgoingArray = nodeSprite.outgoing.map(outgoing => nodeContainer.idIndexMap.indexFrom(outgoing.toId));
